# Documentation Index

Hydra Core is the public governance and failure-control specification for Hydra
Systems trading infrastructure. Architecture defines durable roles and
invariants; the dated posture alone defines changing availability and
public-user permission.

## Start Here

1. [README](README.md)
2. [System Overview](architecture/system-overview.md)
3. [Control Boundaries](architecture/control-boundaries.md)
4. [Multi-Axis State Model](architecture/state-model.md)
5. [Failure Modes](governance/failure-modes.md)
6. [Threat Model](governance/threat-model.md)
7. [Current Public Posture](status/public-operating-posture.md)

## Architecture And Authority

- [Hydra Systems Hierarchy](architecture/hydra-ecosystem.md)
- [Public Surface Registry](architecture/public-surface-registry.md)
- [Hydra Quant](architecture/hydra-quant.md)
- [Hydra Guardian](architecture/hydra-guardian.md)
- [Control Boundaries](architecture/control-boundaries.md)
- [Multi-Axis State Model](architecture/state-model.md)
- [Glossary](glossary.md)

## Failure, Evidence And Recovery

- [Failure Modes](governance/failure-modes.md)
- [Threat Model](governance/threat-model.md)
- [Public Evidence Policy](governance/public-evidence-policy.md)
- [Risk Event Ledger](governance/risk-event-ledger.md)
- [Risk Event Ledger Policy](governance/risk-event-ledger-policy.md)
- [Risk Event Review Process](governance/risk-event-review-process.md)
- [Risk Event Schema Contract](governance/risk-event-schema.md)
- [Risk Event JSON Schema](schemas/risk-event.schema.json)
- [Synthetic Risk Event Examples](examples/risk-events/README.md)
- [Operating Principles](operations/operating-principles.md)
- [Operator Runbook](operations/operator-runbook.md)

## Public Posture And Alpha

- [Public Operating Posture](status/public-operating-posture.md)
- [Machine-Readable Public Posture](status/public-operating-posture.json)
- [Public Posture Schema](schemas/public-operating-posture.schema.json)
- [Hydra Quant Private-Alpha Contract](product/hydra-quant-private-alpha.md)
- [Hydra Control Room Concept](product/control-room-concept.md)

## Doctrine

- [Risk Doctrine](doctrine/risk-doctrine.md)
- [Why Trading Bots Fail](doctrine/why-most-bots-fail.md)

## Release And Repository Policy

- [Release Posture](governance/release-posture.md)
- [Versioning Policy](governance/versioning-policy.md)
- [Changelog](CHANGELOG.md)
- [Proposed v0.1.0-pre-alpha.2 Notes](release-notes/v0.1.0-pre-alpha.2.md)
- [Security Policy](SECURITY.md)
- [Contributing Guide](CONTRIBUTING.md)

Use the exact contract version or commit when relying on these specifications.
No architecture document substitutes for the dated public posture, and no
historical record grants current execution authority.
